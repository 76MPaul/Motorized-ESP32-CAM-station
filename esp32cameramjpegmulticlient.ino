/*
  ...
  MY EDIT DISCLAMER
  Added the MQTT connection, servo part and DHT
*/
...
void initServo() 
{
  servo1.attach(ServoPin1,2); 
  servo2.attach(ServoPin2,3); 
  servo1.write (initial_position1);
  servo2.write (initial_position2);
}

// ------------ DHT ------------
#include "DHTesp.h"          // Librairie des capteurs DHT
...
#define temperature_topic "sensor/temperature/salon"  //Topic température
#define humidity_topic "sensor/humidity/salon"        //Topic humidité
#define error_topic "sensor/error/salon"        //Topic error
#define slider1_topic "sensor/salon/slider1"  //Topic température
#define slider2_topic "sensor/salon/slider2"  //Topic température
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str(), mqtt_user, mqtt_password)) {
      ...
      client.publish("meteor/sensor/test", "hello world");
      // ... and resubscribe
      client.subscribe(slider1_topic);
      client.subscribe(slider2_topic);
    } else {
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (now - lastMsg > 1000 * timer) {
      lastMsg = now;

      TempAndHumidity newValues = dht.getTempAndHumidity();
      if (dht.getStatus() != 0) {
        Serial.println("DHT22 error status: " + String(dht.getStatusString()));
        String msg = "DHT22 error status: " + String(dht.getStatusString());
        client.publish(error_topic, msg.c_str(), true);   //Publie la température sur le topic temperature_topic
        return;
      }
      if ( debug ) {
      ...
      client.publish(temperature_topic, String(newValues.temperature).c_str(), true);   //Publie la température sur le topic temperature_topic
      client.publish(humidity_topic, String(newValues.humidity).c_str(), true);      //Et l'humidité
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  int j = 0;
  if (strcmp(topic, slider1_topic) == 0){
    int topos = msgString.toInt();
    if ( debug ) {
      Serial.print("slider1 value : ");
      Serial.println(topos);
      Serial.print("Current1 value : ");
      Serial.println(pos);
    }
    if(pos < topos){
      for(j = pos; j <= topos; j += 1){
        servo1.write(j);
		delay(25);
      }
    }else{
  ...
  else if (strcmp(topic, slider2_topic) == 0){
    int topos = msgString.toInt();
    if ( debug ) {
      Serial.print("slider2 value to : ");
      Serial.println(topos);
      Serial.print("Current2 value : ");
      Serial.println(pos2);
    }
    if(pos2 < topos){
      for(j = pos2; j <= topos; j += 1){
        servo2.write(j);
		delay(25);
      }
    }else{
